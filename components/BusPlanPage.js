...
export default function BusPlanPage() {
    ...
    const getBusPlanOptedCampers = async () => {
        try {
            setIsLoading(true);
            const response = await axiosPrivate.get(`/api/camps/sessions/${sessionId}/bus-plan-campers/`);
            ...
            const busPlan = response2.data.events.find((event) => event.event === "bus_plan")
            console.log(busPlan);
            const today = new Date();
            const todaysDateString = today.toISOString().split('T')[0];
            const checked_in = busPlan[todaysDateString]["checked_in"];
            console.log("bus plan checked_in");
            ...
            console.log("bus plan campers to be checked in");
            console.log(campers);
    ...
    const getSession = async () => {
        try {
            ...
            const busPlan = response.data.events.find((event) => event.event === "bus_plan")
            console.log(busPlan);
            const today = new Date();
            const todaysDateString = today.toISOString().split('T')[0];
            const checked_in = busPlan[todaysDateString]["checked_in"];
    ...
    async function readNdef() {
        try {
            ...
            axiosPrivate.post(`/api/camps/sessions/${sessionId}/log-plan/${tag.id}`, JSON.stringify(
                {
                    attendanceType: 'bus-plan',
                }
            )).then((response) => {
                setIsLoading(true);
                console.log(response.data);
                getBusPlanOptedCampers();
                setIsLoading(false);
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.header}>
                Total Campers : {totalCampers}, Bus Plan : {checkedInCampers}
            </Text>
            <FlatList
